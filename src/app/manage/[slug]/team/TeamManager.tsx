  ...
  return <div className={styles.workspace}>
    <section className={styles.editor} aria-label={editingId ? "Edit team member" : "Invite team member"}>
      ...
      <form className={styles.form} onSubmit={submit}>
        ...
        <fieldset className={styles.permissions} disabled={cannotEditOwner}><legend>Final permissions</legend>{allCapabilities.map((capability) => { const info = capabilityInfo[capability]; const grantable = actorCapabilities.includes(capability); const checked = draft.capabilities.includes(capability); return <label className={styles.permission} key={capability}><input type="checkbox" checked={checked} disabled={(!grantable && !checked) || cannotEditOwner} onChange={() => toggleCapability(capability)} /><span><strong>{info.label}{info.sensitive ? <em>Sensitive</em> : null}</strong><small>{info.description}{!grantable ? checked ? " You may remove, but cannot re-grant, this permission." : " You cannot grant this permission." : ""}</small></span></label>; })}</fieldset>
