...
export default async function ManagementHomePage() {
  ...
  const membershipPath = (membership: (typeof memberships)[number]) => membership.capabilities.includes("view_orders")
    ? `/manage/${membership.restaurantSlug}/orders`
    : membership.capabilities.includes("manage_memberships")
      ? `/manage/${membership.restaurantSlug}/team`
      : `/manage/${membership.restaurantSlug}/settings`;
  if (memberships.length === 1) redirect(membershipPath(memberships[0]));
    ...
    <main className={styles.main}>
      <section className={styles.restaurantList}>
        ...
        {memberships.map((membership) => (
          <Link className={styles.restaurantLink} key={membership.membershipId} href={membershipPath(membership)}>
