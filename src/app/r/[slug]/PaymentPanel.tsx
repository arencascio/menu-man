  ...
  return (
    <main className={styles.page} aria-label="Order payment">
      <section className={styles.checkoutPanel} aria-live="polite">
        ...
        {payment.status === "authorized" && <div className={styles.paymentWarningPanel}><h3>Payment authorized</h3><p>The payment has not been captured. Do not submit another payment.</p></div>}
        {terminalFailure && (
        ...
        {isLateSuccess && <div className={styles.paymentWarningPanel}><h3>Payment confirmed after cancellation or expiration</h3><p>This order was not placed automatically. It requires restaurant review or a refund.</p></div>}
        ...
        {error && payment.status === "processing" && <p className={styles.formError} role="alert">{error}</p>}
        {(terminalFailure || isExpired) && <Link className={styles.checkoutButton} href={`/r/${restaurantSlug}/checkout`}>Edit Cart and Start Fresh</Link>}
