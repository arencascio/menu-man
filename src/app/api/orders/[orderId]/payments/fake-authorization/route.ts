...
import { fakeAuthorizationActionRequestSchema } from "@/lib/payments/contracts";
import { getGuestPaymentCapability } from "@/lib/payments/capability-cookie";
import { PaymentServerError, resolveFakeAuthorizationAction } from "@/lib/payments/server";
  ...
  try {
    ...
    if (contentLength > 2048) {
      return NextResponse.json(
        { error: { code: "INVALID_REQUEST", message: "Authorization request is too large." } },
        { status: 413, headers },
      );
    }
    const body = fakeAuthorizationActionRequestSchema.parse(await request.json());
    ...
    const payment = await resolveFakeAuthorizationAction(
      orderId,
      checkoutToken,
      body.action,
    ...
    if (error instanceof ZodError || error instanceof SyntaxError) {
      return NextResponse.json(
        { error: { code: "INVALID_REQUEST", message: "Authorization request is invalid." } },
        { status: 400, headers },
      );
    }
    ...
    return NextResponse.json(
      { error: { code: "PAYMENT_FAILED", message: "The authorization could not be updated." } },
      { status: 500, headers },
    );
  }
}
