...
import { composeMenuSections } from "./sections";
...
test("Featured and Favorites precede unchanged source sections and reuse canonical items", () => {
  ...
  assert.deepEqual(result.map((section) => section.id), ["featured", "favorites", "source-1", "source-2"]);
