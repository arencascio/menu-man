...
test("restaurant settings use dedicated authorized routes and preserve the notification integration", () => {
  ...
  const deliveryRoute = source("src", "app", "api", "manage", "restaurants", "[slug]", "settings", "delivery", "route.ts");
  const hoursRoute = source("src", "app", "api", "manage", "restaurants", "[slug]", "settings", "hours", "route.ts");
  assert.match(nav, />Settings</);
  for (const label of ["Restaurant", "Ordering", "Delivery", "Hours", "Notifications"]) assert.match(settingsNav, new RegExp(`"${label}"`));
  ...
  assert.match(settingsServer, /update_managed_delivery_settings_v1/);
  assert.match(settingsServer, /update_managed_hours_settings_v2/);
  for (const route of [restaurantRoute, orderingRoute, deliveryRoute, hoursRoute]) {
