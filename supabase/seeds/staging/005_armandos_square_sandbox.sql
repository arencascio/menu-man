...
\else
  \echo 'Missing -v square_merchant_id=<Square Sandbox merchant ID>'
  \quit
\endif
\if :{?square_location_id}
\else
  \echo 'Missing -v square_location_id=<Square Sandbox location ID>'
  \quit
\endif
...
insert into public.payment_provider_references (
  connection_id, provider_key, environment, reference_kind, external_id, is_primary
)
select connection.id, 'square', 'sandbox', reference.reference_kind, reference.external_id, true
from public.restaurant_payment_connections connection
join public.restaurants restaurant on restaurant.id = connection.restaurant_id
